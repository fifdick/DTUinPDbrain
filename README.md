...
Direct access to result statistics, raw counts (in TPM) and sample metadata (covariates used in the analysis) are available at: [Figshare](https://figshare.com/articles/dataset/Differential_transcript_usage_in_the_Parkinson_s_disease_brain/12941945).
The main analysis was performed with
...
cd DTU_in_PD_brain
tar -zxvf ./results/rds/obj.tar.gz
...
* Meta data including all covariates used in the model design are supplied in the .csv in `./metaData`. The sample IDs match the subdirectory names in `./rawData/`. The cell type estimations listed as covariates ("Microglia\_Genes" and "Oligo\_Genes") have been estimated as described in the paper. The code for the calculation can be found on: [gitApp](https://git.app.uib.no/neuromics/cell-composition-rna-pd). The estimation has be performed on the replication and discovery cohort separately, and all samples of each cohort where included.  
* The log of final run of the pipeline can be found in `./logs`.
* The R object that holds all results is compressed in `./results/rds`. It consists of all data necessary to generate tables and figures presented in the paper (except the qPCR wetlab and DGE results). It was generated by running the script `./runDTU.sh`.
All arguments and parameters that were used to generate these results can be found in the log file `./log/04-05-20.txt`. We also need the result dataframe from the DTU analysis we ran without the covariates `Microglia_Genes` and `Oligo_Genes`. We provide only the dataframe as opposed to the whole result object, due to storage limitations. However, this can be generated by running `./runDTU.sh` with the parameter `batch="rin:sex:age_years"` instead of `batch="rin:sex:age_years:Microglia_genes:Oligo_Genes"`.
