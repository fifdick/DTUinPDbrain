...
tiff(paste0(plotOutDir, "Fig3.tiff"), width = 7, heigh = 8.75, units = "in", res = 300, compression = "lzw") 
grid.arrange(p1, p2, p3)
dev.off()
```
...
tiff(paste0(plotOutDir,"S2Fig1.tiff"),width = 10, height = 13, unit = "in", res = 300, compression = "lzw" )
...
combined
tiff(paste0(plotOutDir,"Fig4.tiff"),width=10,height=10,unit = "in", res = 300)
...
combined
tiff(paste0(plotOutDir,"S2Fig2.tiff"), width = 10, height = 10, unit = "in", res = 300)
combined / guide_area() + plot_layout(guides = "collect", widths = c(3,1))
dev.off() 
```
...
tiff(paste0(plotOutDir,"Fig6.tiff"),width=12,height=12, unit = "in", res = 300)
...
tiff(paste0(plotOutDir,"Fig1.tiff"), width = 7, height = 9, res = 300, units = "in")
grid.arrange(textGrob("A"),gTree(children=venn),resultPlotBiotype,layout_matrix=lay)
dev.off()
```
...
tiff(paste0(plotOutDir,"Fig5.tiff"), compression = "lzw", res = 300, width =7, height = 8.75, units = "in")
grid.arrange(twoTx,OneTx)
dev.off()
```
...
tiff(paste0(plotOutDir,"./S2Fig3.tiff"), width=9, height=6, units = "in", res = 300, compression = "lzw")
((as.ggplot(grobTree(venn.drim_g)) + labs(tag="A")) + (as.ggplot(grobTree(venn.dex_g))) + labs(tag="B")) / (( as.ggplot(grobTree(venn.drim)) + labs(tag="C") )+ (as.ggplot(grobTree(venn.dex)) + labs(tag="D")))
dev.off()
```
...
allDTU_long %>% 
	dplyr::mutate(tx_pvalueStageR = case_when(
						  is.na(tx_pvalueStageR) ~ "1",
						  TRUE ~ "< 0.05")) %>%
	ggplot(., aes(y = paste0(gene_name, " ", tx_id), x = model, fill = as.factor(tx_pvalueStageR))) +
		geom_tile() +
		scale_fill_viridis_d() +
		scale_x_discrete(limits = c("cc", "no_cc", "cc_dex", "no_cc_dex")) -> A
# Find real nom pval and l2fc for NAs
allDTU_long %>% 
	dplyr::mutate(nom_pval = case_when(is.na(nom_pval) ~ 1,
						  TRUE ~ nom_pval)) %>%
	ggplot(., aes(y = paste0(gene_name, " ", tx_id), x = model, fill = -log10(nom_pval))) +
		geom_tile() +
		scale_fill_viridis_c() +
		scale_x_discrete(limits = c("cc", "no_cc", "cc_dex", "no_cc_dex")) -> B
allDTU_long %>% 
	dplyr::mutate(l2fc = case_when(is.na(l2fc) ~ 0,
						  TRUE ~ l2fc)) %>%
	ggplot(., aes(y = paste0(gene_name, " ", tx_id), x = model, fill = l2fc)) +
		geom_tile() +
		scale_fill_viridis_c(option = "C") +
		scale_x_discrete(limits = c("cc", "no_cc", "cc_dex", "no_cc_dex")) -> C
...
# uncomment
# write.table(S2, paste0(tableOutDir, "S1T2.txt"), quote = F, row.names = F, sep = "\t")
```
