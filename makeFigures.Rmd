...
# Read result object:
obj <- readRDS("./results/rds/obj.rds")
...
gene_info <- DTU::get_gene_info(DRIMSeq::counts(obj$Ds_unfilt$Ds_drim$discovery)$feature_id, tx = TRUE) %>%
	dplyr::select(tx_biotype, tx_id)
# discovery cohort 
CPMs <- DRIMSeq::counts(Ds_unfilt$Ds_drim_unfilt$discovery) %>%
