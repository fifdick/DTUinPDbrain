...
# load:
# a list with one dataframe for each cohorts. Each dataframe holds the results of stageR
stageRList<- readRDS("./results/rds/objs_byCohort.rds")
# a list with one dataframe for each cohort. The dataframes contain the DTU events which we identified by both tools
toolIntersection<-  readRDS("./results/rds/drim_dex_toolInt.rds")
# a list of dataframes, holding all DEXSeq result data. One for each cohort
DEXSeqResultDf<- readRDS("./results/rds/dexRes.rds")
# a list of dataframes, holding all DRIMSeq result data. One for each cohort
DRIMSeqResultDf<- readRDS("./results/rds/drimRes.rds")
# a nested list: outer one is a list of tools the inner lists the cohors. 
# i.e. a list of data objects for each tool and each cohort
DRIMSeqDEXSeqObjs<- readRDS("./results/rds/Ds.rds")
# listing the parameters which were used to filter low expressed genes and transcripts 
FilterParam <- readRDS("./results/rds/filtInfo.rds")
# same nested list of data objects, but only for DRIMSeq, prior to filtering.
# As filtering is done with DRIMSeq and then the filtered data passed to DEXSeq.
DRIMSeqDEXSeqObjsPriorFilter<- readRDS("./results/rds/Ds_unfilt.rds")
```
...
```{r createMainDf}
#Those two dataframes are the only ones required by the following chunks. (and ofc the ones loaded above)
df_disc <- DTU::stage_res_plus_info(genelists = stageRList, ds = DRIMSeqDEXSeqObjs, cohort = "discovery") 
df_repl <- DTU::stage_res_plus_info(genelists = stageRList, ds = DRIMSeqDEXSeqObjs, cohort = "replication")
info <- DTU::create_sample_info(pheno = metaDataFile, cohort_names = c("discovery","replication"), conditions=c("Control","Case")) 
obj <- list(dge = dge_obj, dge_results = dge_results, main_df = list("discovery" = df_disc, "replication" = df_repl),
	    Ds = DRIMSeqDEXSeqObjs, res_drim = DRIMSeqResultDf, res_dex = DEXSeqResultDf, stageRList = stageRList, info = info)
...
print(FilterParam$filtInfo_drim$discovery)
print("Filter parameters replication cohort:")
print(FilterParam$filtInfo_drim$replication)
```
...
gene_info <- DTU::get_gene_info(DRIMSeq::counts(DRIMSeqDEXSeqObjsPriorFilter$Ds_drim$discovery)$feature_id, tx = TRUE) %>%
	dplyr::select(tx_biotype, tx_id)
# discovery cohort 
CPMs <- DRIMSeq::counts(DRIMSeqDEXSeqObjsPriorFilter$Ds_drim_unfilt$discovery) %>%
	dplyr::rename(tx_id = feature_id) %>% 
	dplyr::mutate_at(grep("\\.", colnames(.)), ~. / sum(.) * 1000000)

CPMs_filtered <- CPMs %>% dplyr::filter(tx_id %in% DRIMSeq::counts(DRIMSeqDEXSeqObjs$Ds_drim$discovery)$feature_id)
...
# DRIMSeq
exprMixed_drim <-  DRIMSeq::counts(DRIMSeqDEXSeqObjs$Ds_drim$discovery) %>%
...
geneLevel_drim <- as.data.frame(DRIMSeq::results(DRIMSeqDEXSeqObjs$Ds_drim$discovery)) %>%
...
tx_id <- as.data.frame(stringr::str_split(rownames(DEXSeq::featureCounts(DRIMSeqDEXSeqObjs$Ds_dex$discovery)), ":", simplify = TRUE))[, 2] 
exprMixed_dex <-  as.data.frame(DEXSeq::featureCounts(DRIMSeqDEXSeqObjs$Ds_dex$discovery)) %>%
...
geneLevel_dex <- DEXSeq::perGeneQValue(DEXSeq::DEXSeqResults(DRIMSeqDEXSeqObjs$Ds_dex$discovery, independentFiltering = FALSE))
...
#Basically this is the same as above just that this time we do use all transcripts of PV not only the ones which survived stageR correction
...
#sig in PW
sigPV<- subset(df,sigCol==3)
nrow(sigPV)
cor.test(sign(sigPV$l2fc_disc),sign(sigPV$l2fc_repl))
cor(sigPV$l2fc_disc,sigPV$l2fc_repl)
t.test(as.numeric(sigPV$agreelfc)-1,as.numeric(nonSig$agreelfc)-1)
...
#sig in NBB
sigNBB<- subset(df,sigCol==-2)
nrow(sigNBB)
cor.test(sign(sigNBB$l2fc_disc),sign(sigNBB$l2fc_repl))
cor(sigPV$l2fc_disc,sigPV$l2fc_repl)
t.test(as.numeric(sigPV$agreelfc)-1,as.numeric(nonSig$agreelfc)-1)
```
...
Number of missing genes in NBB due to low expression  
...
```{r Table 3_and_4}
#DEXSeq
#exclue all transcripts which are not present in NBB (NA's)
df <- na.omit(left_join(final_disc_dex,obj$main_df$replication$dex[,c("tx_id", "nom_pval", "l2fc")], by = "tx_id", suffix = c("_disc", "_repl")))
# retain only transcripts where the sign of the l2fc or effect size in PV equals the sign in NBB
df <- df %>% dplyr::filter(sign(l2fc_disc) == sign(l2fc_repl), nom_pval_repl < 0.05) 
#format for latex and sort by PV transcript stageR pvalue
...
lapply(unique(df_drim$gene_name),function(geneName){
	       plot_genes_cohort(gene_name = geneName,
				 tool = "drim",
				 only_nom_sig = F,
				 conditions = c("CT","PD"),
				 out = plotOutDir,
...
#Specifically, it has been reported that 4 splice variants of SNCA are increased in the frontal cortex of PD patients and that \textit{PARK2}  isoforms are differentially expressed in the same brain region
#which are they?
#full length transcript SNCA-140- SNCA-001/2/3/6/8 
#SNCA004 SNCA-203 201 005 abd 202 splice variants
#SNCA-98 vrain specific 
# many more -->13
#SNCA-140, SNCA-126, SNCA-112, and SNCA-98 overexpressed in PD frontal cortex 126 upregulated 
# Some interesting data emerge on SNCA-112 variant. An association between PD risk-associated single nucleotide polymorphisms (SNPs) within the 3′ region of SNCA gene and higher SNCA-112 ratio level has been observed in about 100 of frontal cortex samples

# we only see some DTU in the SNCA-006 (SNCA140) full length transcript (protein_coding) 
# SNCA-112 see effect size of 0.39 *(drim) and 0.36 (dex) but nom pvalue of 0.07 (deX) and 0.2 (drim)
# sig upregulated SNCA126 only: ENST00000394989( not ENST00000505199) not upregulated (0.06 es)


#9 transcripts present prio filtering (all protein coding)
#4 transcripts are present after filtering one transcript shows slight DTU with a nominal p-value of 0.055 and an effect size of -0.49 (drim)

#park2
#11 splicie variants also called parkin
# we find only 7,out of which 2 remian after filtering and none show DTU 
# study with 5 PDS showed 5 expressed transcripts with 3 sig transcirpts with altered raitos in PD post moretm brain tissue of barcelona brain bank, frontal cortex. RNA isolation and realtime PCR 
# other study TV12 and TV3 same study other publicaiton (same authors)
# 6 transcripts present prior filtering 
# 2 transcripts are present after filtering. no transcript shows evidence of DTU (nom pvalues 0.795 and effect siye 0.087)
#
#park7
# no transcripts of PARK7 are present after filtering
# 10 transcripts present prior to filtering all below threshold except ENST00000338639 protein coding. (dtu only considers genes with at least two isoforms present after filtering
...
#define df with significant (after correction) DTU events in PV from both tools
final_disc_dex<- df_disc$dex %>% dplyr::filter(!(is.na(tx_pvalueStageR))) %>% dplyr::select(tx_id,gene_name,gene_id,l2fc,tx_pvalueStageR,nom_pval,tx_biotype) %>% dplyr::arrange(abs(l2fc))
final_disc_drim<- df_disc$drim %>% dplyr::filter(!(is.na(tx_pvalueStageR))) %>% dplyr::select(tx_id,gene_id,gene_name,l2fc,tx_pvalueStageR,nom_pval,tx_biotype) %>% dplyr::arrange(abs(l2fc))



dge <- dge_results$PV %>% dplyr::filter(gene_id %in% c(final_disc_drim$gene_id,final_disc_dex$gene_id))
...
dge <- dplyr::left_join(df,dge,by="gene_id",suffix=c("dge","dtu")) %>% dplyr::group_by(Tool,gene_id) %>% dplyr::add_tally() %>% dplyr::mutate(sig=ifelse(padj<0.05,1,0))
...
with STRINGdb and a background comprised of all genes which were expressed enough to survive filtering.  
...
#subset sig. hits
final_disc_dex<-obj$main_df$PV$dex %>% dplyr::filter(!(is.na(tx_pvalueStageR))) %>% dplyr::select(tx_id,gene_id,gene_name,l2fc,tx_pvalueStageR,nom_pval) %>% dplyr::arrange(abs(l2fc))
final_disc_drim<-obj$main_df$PV$drim %>% dplyr::filter(!(is.na(tx_pvalueStageR))) %>% dplyr::select(tx_id,gene_id,gene_name,l2fc,tx_pvalueStageR,nom_pval) %>% dplyr::arrange(abs(l2fc))
...
PV_bp <- string_db$get_enrichment( hits$STRING_id, category="Process",methodMT = "fdr", iea = TRUE ) %>% dplyr::filter(pvalue_fdr<0.05) %>% tbl_df %>% print(n = Inf)
PV_cc <- string_db$get_enrichment( hits$STRING_id, category="Component",methodMT = "fdr", iea = TRUE ) %>% dplyr::filter(pvalue_fdr<0.05) %>% tbl_df %>% print(n = Inf)
PV_mf <- string_db$get_enrichment( hits$STRING_id, category="Function",methodMT = "fdr", iea = TRUE ) %>% dplyr::filter(pvalue_fdr<0.05) %>%  tbl_df %>% print(n = Inf)
PV_KEGG <- string_db$get_enrichment( hits$STRING_id, category="KEGG",methodMT = "fdr", iea = TRUE ) %>% dplyr::filter(pvalue_fdr<0.05) %>%  tbl_df %>% print(n = Inf)
...
geneDf <- geneDf %>% dplyr::mutate(pvalue=ifelse(is.na(gene_pvalueStageR),1,gene_pvalueStageR))
geneDf <- geneDf %>% dplyr::select(gene_id,pvalue,gene_name) %>% dplyr::rename(gene=gene_name)
...
results_string <- list(PV_bp,PV_mf,PV_cc)
names(results_string) <- categories
objs<- lapply(categories,function(c)(create_input(stringDBRes=results_string[[c]],geneDf=df_disc$drim,stringdb=string_db,categoryTmp=c)))
names(objs) <- categories

#cluster pathways
clusters <- lapply(seq(1,length(objs)),function(i)(run_pathway_vis(input=objs[[i]],plotTitle=categories[[i]],subsetsize=50)))
saveRDS(clusters,paste0(plotOutDir,"./clusters_stringdb_PV.rds"))
clusters <- readRDS(paste0(plotOutDir,"./clusters_stringdb_PV.rds"))

#format dataframes for latex
clusters <- lapply(clusters,function(cl)(
cl <- cl %>% dplyr::mutate(title=paste0("& ",title)) %>% dplyr::group_by(title) %>% dplyr::mutate(qvalue=p.adjust(fisher(pvalue),method="bonferroni")) %>% dplyr::mutate(qvalue_rounded= paste0(formatC(qvalue,format="E",digits=2),"\\\\")) %>% dplyr::arrange(qvalue) %>% dplyr::distinct(title,qvalue_rounded)
))


write.table(clusters[[1]],file=paste0(tableOutDir,"pathwayClusterGObp.txt"),quote=F,row.names=F,col.names=T,sep="&")
write.table(clusters[[2]],file=paste0(tableOutDir,"pathwayClusterGOmf.txt"),quote=F,row.names=F,col.names=T,sep="&")
write.table(clusters[[3]],file=paste0(tableOutDir,"pathwayClusterGOcp.txt"),quote=F,row.names=F,col.names=T,sep="&")
```

### qPCR results for ZNF189 
...
genesToPlot <- get_gene_info(subset(obj$main_df$PV$drim,gene_name %in% lab_genes)$tx_id,tx=T)
genesToPlot
#generate count plots and count dfs for bvoth cohorts for those transcripts
plotsPV<- plot_genes_cohort(obj=obj,cohort="PV",genes_toplot=genesToPlot,tool="drim",plot=F,onlyNomSig=F,WantJitter=F,conditions=c("CT","PD"))  
plotsNBB<- plot_genes_cohort(obj=obj,cohort="NBB",genes_toplot=genesToPlot,tool="drim",plot=F,onlyNomSig=F,WantJitter=F,conditions=c("CT","PD"))  
plots <- append(plotsPV,plotsNBB)
#qPCR samples
qPCR_samples<-list("PV"=c("SL283614","SL283585","SL283596"),"NBB"=c( "SL283581" ,"SL283569", "SL283565"))
#extract counts from plot obj
dfs <- lapply(seq(1,length(plots)),function(i)(subset(plots[[i]]$data,sample_id %in% unlist(qPCR_samples)) %>% dplyr::select(transcript_id,sample_id,condition,frac,tx_id_label,countType)))

#filtered out transcript: "ENST00000496104" it is not expressed in NBB
#index 2 and 4 correspond to ZNF
...
makeLabPlot <-function(df,tool,tag,legend=F) {
 p<-ggplot(subset(df,countType=="lab" | countType=="observed_tx"),aes(x=condition,y=frac)) +
...
#  stat_summary(fun.data="median_sdl",  fun.args = list(mult=1), 
 #             geom="pointrange", color = "red") +
	facet_grid(cols=vars(tx_id_label),rows=vars(source),scales="free_y") +
geom_boxplot(data=subset(df,countType == "fitted_tx") %>% dplyr::group_by(condition,transcript_id) %>% dplyr::mutate(mean=median(frac)),aes(x=condition,y=mean,col="blue"),inherit.aes=F,size=0.2) + 
	labs(x="Condition",y="Relative abundance",title="",tag=tag,col="Median of: ") +
	theme_bw() +
	scale_x_discrete(labels=c("0"="CT","1"="PD")) +
scale_color_manual(values=c("red"="red","blue"="blue"),labels=c("red"="observed values\n (black points)","blue"=paste0("fitted values - ",tool," \n (points not displayed)"))) +
theme(panel.spacing = unit(1, "lines")) +
theme(strip.background=element_rect(fill="black"))+
theme(strip.text= element_text(color="white",face="bold")) 
...
znfSilicoObs<-gdata::combine(dfs[[1]],dfs[[2]]) %>% dplyr::select(transcript_id,frac,condition,tx_id_label,countType)

znfLab <- read.csv(znfLabFile,sep="\t")
colnames(znfLab) <- c("frac","info","condition","transcript_id","tx_id_label","taqman","sybrperc")
znfLab$tx_id_label <- sapply(znfLab$transcript_id,function(id)(paste0(id,"\nprotein_coding")))
znfLab <- znfLab %>% dplyr::select(transcript_id,frac,condition,tx_id_label) %>% dplyr::mutate(condition=ifelse(condition=="CT",0,1))
znfLab$tx_id_label <- as.character(znfLab$tx_id_label)
znfLab$countType<-rep("lab",nrow(znfLab))
dfZNF<-gdata::combine(znfSilicoObs,znfLab,names=c("RNASeq","qPCR")) %>% dplyr::filter(!grepl("dge",countType))



znf<-makeLabPlot(df=dfZNF,tool="DEXSeq",tag="")
#xpa<-makeLabPlot(df=dfXPA,tool="DRIMSeq",legend=T,tag="b")
     
svg(paste0(plotOutDir,"Figure2.svg"))
znf
dev.off()

#grid.arrange(znf,xpa)
...
```{r S3}
stageRList<- readRDS("../Results/general/noCellCorr/rds/objs_byCohort.rds")
toolIntersection<-  readRDS("../Results/general/noCellCorr/rds/drim_dex_toolInt.rds")
DEXSeqResultDf<- readRDS("../Results/general/noCellCorr/rds/dexRes.rds")
DRIMSeqResultDf<- readRDS("../Results/general/noCellCorr/rds/drimRes.rds")
DRIMSeqDEXSeqObjs<- readRDS("../Results/general/noCellCorr/rds/Ds.rds")

FilterParam <- readRDS("../Results/general/noCellCorr/rds/filtInfo.rds")
DRIMSeqDEXSeqObjsPriorFilter<- readRDS("../Results/general/noCellCorr/rds/Ds_unfilt.rds")

df_disc_noCC <- DTU::stageResPlusInfo(genelists=stageRList ,ds=DRIMSeqDEXSeqObjs,cohort="PV") 
df_repl_noCC <- DTU::stageResPlusInfo(genelists=stageRList,ds=DRIMSeqDEXSeqObjs,cohort="NBB")
info<-create_sample_info(pheno=metaDataFile,cohort_names=c("PV","NBB"),conditions=c("Control","Case")) 
obj_noCC<-list(dge=dge_obj,dge_results=dge_results,main_df=list("PV"=df_disc_noCC,"NBB"=df_repl_noCC),Ds=DRIMSeqDEXSeqObjs,res_drim=DRIMSeqResultDf,res_dex=DEXSeqResultDf,stageRList=stageRList,info=info)



#########################################

finalDTU <- obj$main_df$PV$drim %>% 
	dplyr::filter(!(is.na(tx_pvalueStageR))) %>% 
	dplyr::filter(tx_id %in% (subset(obj$main_df$NBB$drim,nom_pval<0.05)$tx_id)) %>% 
	dplyr::filter(sign(l2fc) == sign(obj$main_df$NBB$drim[match(tx_id,obj$main_df$NBB$drim$tx_id),"l2fc"]))


# which of the replicated DTU events were found when not correcting for cell types
#1. in the final set that replicates
finalDTU_noCC <- obj_noCC$main_df$PV$drim %>% 
	dplyr::filter(!(is.na(tx_pvalueStageR))) %>% 
	dplyr::filter(tx_id %in% (subset(obj$main_df$NBB$drim,nom_pval<0.05)$tx_id)) %>% 
	dplyr::filter(sign(l2fc) == sign(obj$main_df$NBB$drim[match(tx_id,obj$main_df$NBB$drim$tx_id),"l2fc"]))

consistent_replicated_indpCC_DTU <- finalDTU[which(finalDTU$tx_id %in% finalDTU_noCC$tx_id),]

#which of the replicated DTU events that were identified when ignoring cell changes, was lost through CC?


finalDTU_noCC[!(finalDTU_noCC$tx_id %in% finalDTU$tx_id),]
finalDTU_noCC[!(finalDTU_noCC$gene_id %in% finalDTU$gene_id),]
...
finalDTUdex <- obj$main_df$PV$dex %>% 
	dplyr::filter(!(is.na(tx_pvalueStageR))) %>% 
	dplyr::filter(tx_id %in% (subset(obj$main_df$NBB$dex,nom_pval<0.05)$tx_id)) %>% 
	dplyr::filter(sign(l2fc) == sign(obj$main_df$NBB$dex[match(tx_id,obj$main_df$NBB$dex$tx_id),"l2fc"]))


# which of the replicated DTU events were found when not correcting for cell types
#1. in the final set that replicates
finalDTU_noCCdex <- obj_noCC$main_df$PV$dex %>% 
	dplyr::filter(!(is.na(tx_pvalueStageR))) %>% 
	dplyr::filter(tx_id %in% (subset(obj$main_df$NBB$dex,nom_pval<0.05)$tx_id)) %>% 
	dplyr::filter(sign(l2fc) == sign(obj$main_df$NBB$dex[match(tx_id,obj$main_df$NBB$dex$tx_id),"l2fc"]))
...
```{r}
PVdrim <- obj$main_df$PV$drim %>%
	    dplyr::select(-rankDex,-toolAdjPval) %>%
	    dplyr::filter(!is.na(gene_pvalueStageR)) %>%
	    dplyr::arrange(gene_name) 
PVdex <- obj$main_df$PV$dex %>%
	    dplyr::select(-rankDrim,-toolAdjPval) %>%
	    dplyr::filter(!is.na(gene_pvalueStageR)) %>%
	    dplyr::arrange(gene_name)  
NBBdrim <- obj$main_df$NBB$drim %>%
	    dplyr::select(-rankDex,-toolAdjPval) %>%
	    dplyr::filter(!is.na(gene_pvalueStageR)) %>%
	    dplyr::arrange(gene_name) 
NBBdex <- obj$main_df$NBB$dex %>%
	    dplyr::select(-rankDrim,-toolAdjPval) %>%
	    dplyr::filter(!is.na(gene_pvalueStageR)) %>%
	    dplyr::arrange(gene_name) 

PW <- dplyr::bind_rows("DRIMSeq"=PVdrim,"DEXSeq"=PVdex,.id="tool")
NBB <-  dplyr::bind_rows("DRIMSeq"=NBBdrim,"DEXSeq"=NBBdex,.id="tool")

S2 <- dplyr::bind_rows("PW"=PW,"NBB"=NBB,.id="cohort")
