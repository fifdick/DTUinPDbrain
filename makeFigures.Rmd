...
biotype_enr <-  function(biotype_df, tool) {
 stats <- lapply(unique(biotype_df$tx_biotype), function(type) {	   
   	 print(type)
	 ...
	 print(tab)
	 # chisq.test(tab)
	 return(data.frame("tx_biotype" = type, "p" = fisher.test(tab)$p, "or" = fisher.test(tab)$estimate))
 })
 names(stats) <- unique(biotype_df$tx_biotype)
