...
tiff(paste0(plotOutDir,"./S2Fig4.tiff"), width=9, height=6, units = "in", res = 300, compression = "lzw")
A + C + B
dev.off()
...
DRIMSeq::counts(obj$Ds$Ds_drim$discovery) %>% 
	...
	dplyr::summarise(infl_rin = cor(value, RIN), med = median(value)) %>% 
	ggplot(., aes(x = 1, y = infl_rin)) +
		geom_violin() +
		...
		labs(tag = "B", title = "Non concordant transcripts", y = "Corr.coeff. RIN and TPM") -> p1
DRIMSeq::counts(obj$Ds$Ds_drim$discovery) %>% 
	...
	ggplot(., aes(x = 1, y = infl_rin)) +
		...
		theme(axis.title.x=element_blank(),
        	      axis.text.x=element_blank(),
                      axis.ticks.x=element_blank()) +
		labs(title = "Concordant transcripts", y = "Corr.coeff. RIN and TPM") -> p2
...
p3 <- draw.triple.venn(ind=F, cat.fontfamily = "sans",
		       ...
		       category = c("Discovery", "Replication", "DTU events \n discovery cohort"),
...
wrap_elements(panel = as.ggplot(gTree(children = p3)) + ggtitle('A')) / (p1 | p2)

tiff(paste0(plotOutDir,"S2Fig5.tiff"), width = 7, height = 9, res = 300, units = "in")
wrap_elements(panel = as.ggplot(gTree(children = p3)) + ggtitle('A')) / (p1 | p2)
dev.off()
